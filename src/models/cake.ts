...
class Cake extends API {
  ...
  async createCake(req: express.Request, res: express.Response) {
    ...
    const imagePath = await s3.upload(formData.file.path,
      formData.file.headers['content-type'] as string,)
    if (false === imagePath) {
      res.statusCode = 500
  ...
  async getCake(req: express.Request, res: express.Response) {
    res.type('application/json')
    const cake = await getCakeQuery(Number.parseInt(req.params.id))
    if (!cake) {
      res.statusCode = 400
      return res.send(JSON.stringify({
  ...
  async delCake(req: express.Request, res: express.Response) {
    res.type('application/json')
    const response = await dropCakeQuery(Number.parseInt(req.params.id))
    if (1 !== response.affectedRows) {
      res.statusCode = 500
      return res.send(JSON.stringify({
        error: 'Unexpected error occurred, please try again.',
  ...
  async updateCake(req: express.Request, res: express.Response) {
    res.type('application/json')
    const oldCake = await getCakeQuery(Number.parseInt(req.params.id))
    if (!oldCake) {
      res.statusCode = 400
      return res.send(JSON.stringify({
        error: 'Resource not found.',
    ...
    const newCake: CakeResult = oldCake
    if (null !== formData.file) {
      const response = await s3.del(oldCake.imageUrl)
      if (false === response) {
        res.statusCode = 500
        return res.send(JSON.stringify({
          error: 'Unexpected error occurred, please try again.',
    ...
    const response = await updateCakeQuery(newCake)
    if (1 !== response.affectedRows) {
      res.statusCode = 500
      return res.send(JSON.stringify({
        error: 'Unexpected error occurred, please try again.',
